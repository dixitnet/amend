...
test('la déconnexion efface la session (le cookie renvoyé ne revérifie plus)', async () => {
  ...
  assert.deepEqual(await beforeRes.json(), { email, admin: false, nom: null, couleur: null })
  ...
  assert.deepEqual(await afterRes.json(), { email: null, admin: false, nom: null, couleur: null })
})
